...
class report extends \mod_scorm\report {
    ...
    public function display($scorm, $cm, $course, $download) {
        ...
        if ( !$nostudents ) {
			...
			$columns[] = 'profile_field_department';
            $headers[] = 'Department';
			
			$columns[] = 'profile_field_division';
            $headers[] = 'Division';
			
			$columns[] = 'profile_field_group';
            $headers[] = 'Group';
			/*********** added by Pankaj Wagh ******** dt. 02/09/2021 ************/
            $columns[] = 'attempt';
		...
		//	print_object($columns);
            ...
            if (!$download) {
				...
				/*********** added by Pankaj Wagh ******** dt. 19/09/2021 ************/
                $table->no_sorting('profile_field_department');
                $table->no_sorting('profile_field_division');
                $table->no_sorting('profile_field_group');
            ...
            if ($attempts) {
                foreach ($attempts as $scouser) {
					...
					$sql ='SELECT f.id, f.shortname, f.datatype, d.data
                                           ...
                                           WHERE d.userid = ?';
					$user_profiles = $DB->get_records_sql($sql, array( $scouser->userid));
					$profileuser = new \stdclass();
					foreach($user_profiles as $user_profile)
					{
						$key = 'profile_field_'.$user_profile->shortname;
						$profileuser->$key= $user_profile->data;
					}
					
					
					
					$row[] = s($profileuser->profile_field_department);
					$row[] = s($profileuser->profile_field_division);
					$row[] = s($profileuser->profile_field_group);
					/*********** added by Pankaj Wagh ******** dt. 02/09/2021 ************/
                    ...
                    if (empty($timetracks->start)) {
						...
						/*if($scouser->attempt > 0 && $strstatus ==get_string('notattempted', 'scorm'))
						{
							$strstatus =get_string('incomplete', 'scorm');
						}*/
                        $row[] = $strstatus;
                    }
