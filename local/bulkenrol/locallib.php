                ...
                if (!empty($courseid) && !empty($userstoenrol)) {
                    try {
                        ...
                        if (empty($enrolinstance)) {
							...
							/********** added by Pankaj Wagh **** 08/Sept/2021 ***********/
						
							if( $SESSION->enroltimestart > 0 &&  $SESSION->enroltimeend > 0)
							{
								$enrolinstance->enrolstartdate = $SESSION->enroltimestart;
								$enrolinstance->enrolenddate = $SESSION->enroltimeend;
							}
							/********** added by Pankaj Wagh **** 08/Sept/2021 ***********/
                            ...
                            foreach ($userstoenrol as $user) {
                                try {
                                    ...
                                    if ($userisenrolled) {
										...
										if( $SESSION->enroltimestart > 0 &&  $SESSION->enroltimeend > 0)
										{
											$plugin->enrol_user($enrolinstance, $user->id, $roleid,$SESSION->enroltimestart,$SESSION->enroltimeend);//added by Pankaj Wagh - 08/Sept/2021
										}
										else
										{
											$plugin->enrol_user($enrolinstance, $user->id, $roleid);//added by Pankaj Wagh - 08/Sept/2021
										}
                                    }
                                } catch (Exception $e) {
                                    $a = new stdClass();
