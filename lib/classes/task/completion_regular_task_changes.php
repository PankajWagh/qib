        ...
        if ($CFG->enablecompletion) {
            ...
            $sql = 'SELECT DISTINCT c.id AS course, cr.id AS criteriaid, crc.userid AS userid,
                                    cr.criteriatype AS criteriatype, cc.timecompleted AS timecompleted,
									 crc.timecompleted AS crctimecompleted
                ...
                INNER JOIN {course_completions} crc ON crc.course = c.id
                 LEFT JOIN {course_completion_crit_compl} cc ON cc.criteriaid = cr.id AND crc.userid = cc.userid
                     WHERE c.enablecompletion = 1
                       ...
                       AND (crc.reaggregate > 0 or (cc.timecompleted IS NULL and c.id = 51) ) 
            ...
            while (1) {
                ...
                if (!empty($completions)) {
					...
					if(!empty($params->crctimecompleted) && empty($overallstatus ))
					{
						$overallstatus = true;
					}
					mtrace(__LINE__."******************************".$overallstatus);
                    ...
                    if ($overallstatus) {
						...
						$sql ="select * from {course_completions} where course =$params->course and userid=$params->userid and timecompleted is NULL";
						$rs_completion = $DB->get_record_sql($sql, array());
						if(isset($rs_completion->id))
						{
							$ccompletion->id =$rs_completion->id;
							$ccompletion->timeenrolled =$rs_completion->timeenrolled;
							$ccompletion->timestarted =$rs_completion->timestarted;
							$ccompletion->reaggregate =$params->crctimecompleted;
							$ccompletion->timecompleted =$params->crctimecompleted;
							$timecompleted = $params->crctimecompleted;
						}
						...
						/************************** Course Completion Pankaj Wagh dt 11/Sept/2021 **************/
						 $supportuser = \core_user::get_support_user(); 
						 global $DB;
						 $user = $DB->get_record('user', array('id' => $params->userid));	
						 $course = $DB->get_record('course', array('id' =>$params->course));	
						 $message = 'Dear '.fullname($user, true).', <br>Congratulations, you have completed the assignment course "'.$course->fullname.'".
						 ...
						 mtrace($message);
						 mtrace($subject);
						 email_to_user($user, $supportuser, $subject, nl2br($message));
						/************************** Course Completion Pankaj Wagh dt 11/Sept/2021 **************/
                    }
                }
