...
function get_enrolled_join(context $context, $useridcolumn, $onlyactive = false, $onlysuspended = false, $enrolid = 0) {
    ...
    if (!$isfrontpage) {
        ...
        $where2 = "{$prefix}ue.timestart < :{$prefix}now1 AND ({$prefix}ue.timeend = 0 OR {$prefix}ue.timeend > :{$prefix}now2 or {$prefix}ue.timeend <= UNIX_TIMESTAMP())";
...
abstract class enrol_plugin {
    ...
    public function update_instance($instance, $data) {
        ...
        $properties = array('status', 'name', 'password', 'customint1', 'customint2', 'customint3',
							...
							'expirythresholdfrequency');//added by Pankaj Wagh 08/10/2021
    ...
    public function send_expiry_notifications($trace) {
      ...
      if ($expirynotifylast > $notifytime) {
        ...
        $sql = "SELECT ue.*, e.expirynotify, e.notifyall, e.expirythreshold, e.courseid, c.fullname
                 ...
                 WHERE ue.status = :active AND ue.timeend > 0 AND ue.timeend > :now1 AND (ue.timeend < (e.expirythreshold + :now2) or ( ue.timeend > (e.expirythreshold + :now3) and  mod(DATEDIFF(FROM_UNIXTIME(e.expirythreshold + :now4),curdate()), if(e.expirythresholdfrequency,e.expirythresholdfrequency,(e.expirythreshold/846000)))=0))
				 and u.id not in (select userid from {course_completions} cc where cc.course = c.id and timecompleted is not NULL )
              ORDER BY ue.enrolid ASC, u.lastname ASC, u.firstname ASC, u.id ASC";
        $params = array('enabled'=>ENROL_INSTANCE_ENABLED, 'active'=>ENROL_USER_ACTIVE, 'now1'=>$timenow, 'now2'=>$timenow, 'now3'=>$timenow, 'now4'=>$timenow, 'name'=>$name);
		...
		$enrollerflag = false;
        foreach($rs as $ue) {
            if ($lastenrollid and $lastenrollid != $ue->enrolid and ($ue->expirynotify==2 || $ue->expirynotify==1)) {
				
					$trace->output("___Enroller_____".__LINE__);
					$this->notify_expiry_enroller($lastenrollid, $users, $trace);
				
                $users = array();
            }
            $lastenrollid = $ue->enrolid;
			if($ue->expirynotify==2 || $ue->expirynotify==1)
			{
				$enrollerflag = true; 
			}
			...
			$trace->output("____________".fullname($user)."**********".$ue->expirynotify , 1);
          ...
          /* if (!$ue->notifyall) {
           ...
           /* if ($ue->timeend - $ue->expirythreshold + 86400 < $timenow) {
			...
			if($ue->expirynotify==2 || $ue->expirynotify==3) 
			{
				$trace->output("___Enrolled_____".__LINE__);
        ...
        if ($lastenrollid and $users && $enrollerflag) {
			$trace->output("___Enroller_____");
            $this->notify_expiry_enroller($lastenrollid, $users, $trace);
        }
    ...
    protected function notify_expiry_enroller($eid, $users, progress_trace $trace) {
		...
		$enddate=$info['timeend'];
		}
		
		$trace->output(var_dump($users));
		$trace->output(time() ."***********".$enddate , 1);
		if(time() < $enddate )
		{
			$endate = date("Y-m-d",$enddate);
			$currdate = date("Y-m-d");
			$date1=date_create($endate);
			$date2=date_create($currdate);
			$diff=date_diff($date1,$date2);
			$trace->output("____________".$diff->format("%a") , 1);
			$a->threshold=  $diff->format("%a Days");
		}
		 foreach($users as $key=>$info) {
