...
class enrol_notificationeabc_plugin extends enrol_plugin
{
    ...
    public function process_mensaje($mensaje, stdClass $user, stdClass $course) {
		...
		$userenrolment = $DB->get_record('user_enrolments',['enrolid' => $manualinstance->id, 'userid' =>$user->id]);
		...
		/***************added by pankaj wagh *********** 02/09/2021 ******/
		$manualinstance = $DB->get_record('enrol', ['courseid' => $course->id, 'enrol' => 'manual'], '*', MUST_EXIST);
		$userenrolment = $DB->get_record('user_enrolments',['enrolid' => $manualinstance->id, 'userid' =>$user->id]);
		$m = str_replace('{TIMEEND}', $userenrolment->timeend ==0?'':userdate($userenrolment->timeend, get_string('strftimedatetimeshort', 'langconfig')), $m);
		/***************added by pankaj wagh *********** 02/09/2021 ******/
        return $m;
    }
