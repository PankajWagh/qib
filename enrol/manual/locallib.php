...
class enrol_manual_potential_participant extends user_selector_base {
    protected $enrolid,$group,$division,$department;
    ...
    public function find_users($search) {
		...
		global $DB;
				if(trim($this->department) !='')
				{
					$wherecondition .=' and u.id in (SELECT d.userid
                                            FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
                                           WHERE f.shortname="department" and d.data=:department)					   
					';
				...
				if(trim($this->division) !='')
				{
					$wherecondition .=' and u.id in (SELECT d.userid
                                            FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
                                           WHERE f.shortname="division" and d.data=:division)					   
					';
				...
				if(trim($this->group) !='')
				{
					$wherecondition .=' and u.id in (SELECT d.userid
                                            FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
                                           WHERE f.shortname="group" and d.data=:group)					   
					';
