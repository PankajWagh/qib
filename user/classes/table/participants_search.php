...
class participants_search {
    ...
    public function get_participants(string $additionalwhere = '', array $additionalparams = [], string $sort = '',
        ...
        $sql = "{$outerselect},if(timestart> 0,FROM_UNIXTIME(timestart),'') as timestart,if(timeend> 0,FROM_UNIXTIME(timeend),'') as timeend
                          FROM ({$innerselect}
                                          FROM {$innerjoins}
                                 {$innerwhere}
                               ) {$subqueryalias}
    ...
    protected function get_participants_sql(string $additionalwhere, array $additionalparams): array {
        ...
        $innerselect = "SELECT DISTINCT {$inneruseralias}.id,timestart,timeend";
        ...
        if ($this->filterset->has_filter('keywords')) {
		 ...
		 if ($this->filterset->has_filter('coursestatus')) 
		{
           [
                'where' => $coursestatuswhere,
			] = $this->get_coursestatus_sql();
			
			if(trim($coursestatuswhere) !='')			
			$wheres[] =$coursestatuswhere; 
        }
	...
	protected function get_coursestatus_sql(){
		$where = '';
		global $DB;
		
		$courseid = $this->filterset->get_filter('courseid')->get_filter_values()[0];
		
		
		$sqlcoursescorm ='SELECT  distinct userid from {scorm_scoes_track} sst inner join {scorm} scm on sst.scormid = scm.id where element="cmi.core.lesson_status" and lower(value) in ("passed","completed") and  scm.course=? ';
		$recs = $DB->get_records_sql($sqlcoursescorm,Array($courseid));
		$user_arr = array();
		...
		$sqlcoursecompletion ='SELECT  distinct userid from {course_completions} where course=? and timecompleted is not NULL';
		$recs = $DB->get_records_sql($sqlcoursecompletion,Array($courseid));
		
		foreach($recs as $rec)
		{
			if(!in_array($rec->userid,$user_arr))
			$user_arr[]=$rec->userid;
		}
		...
		if ($this->filterset->has_filter('coursestatus')) {
                $cousesstatusjointype = $this->filterset->get_filter('coursestatus')->get_join_type();
                $cousestatusfiltervalues = $this->filterset->get_filter('coursestatus')->get_filter_values();

                // If values are set for the status filter, use them.
                if (!empty($cousestatusfiltervalues)) {
                    $coursestatus = $cousestatusfiltervalues;
                }
				
				 if($coursestatus[0] ==0)
				 {
					 $where ='  u.id in ('.implode(",",$user_arr).')' ;
				 }
				 elseif($coursestatus[0] ==1)
				 {
					  $where ='  u.id not in ('.implode(",",$user_arr).')' ;
				 }
			//	echo "*****************".__LINE__."********".$courseid."*****".implode(",",$user_arr);
            }
