...
M.core_user.init_user_selector = function (Y, name, hash, extrafields, lastsearch) {
    ...
    var user_selector = {
        ...
        send_query : function(forceresearch) {
			...
			var department = division = group ='';
			if($("#menudepartment"))
			department = $("#menudepartment").val();
			if($("#menudivision"))
			division = $("#menudivision").val();
			if($("#menugroup"))
			group = $("#menugroup").val();
			
			console.log(department+"***"+division+"***"+group+"***");

            var iotrans = Y.io(M.cfg.wwwroot + '/user/selector/search.php', {
                method: 'POST',
                data: 'selectorid=' + hash + '&sesskey=' + M.cfg.sesskey + '&search=' + value + '&userselector_searchanywhere=' + this.get_option('searchanywhere')+'&department='+department+'&division='+division+'&group='+group,
                on: {
