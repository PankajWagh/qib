...
class raw_event_retrieval_strategy implements raw_event_retrieval_strategy_interface {
    ...
    protected function get_raw_events_legacy_implementation(
		...
		$module =$DB->get_record_sql("select * from {modules} where name='googlemeet' ");
		
		$mycourses = enrol_get_my_courses();
		foreach($mycourses  as $mycourse)
		{
			$sql = "select * from {googlemeet} where course=".$mycourse->id;
			$googlemeets = $DB->get_records_sql($sql);
			...
			foreach($googlemeets as $googlemeet)
			{
				...
				$sql ="select * from {event} where modulename='googlemeet' and  instance='".$googlemeet->id."' and 
					   timemodified <='".$googlemeet->timemodified."' and  timesort > now()";
					   
				$googlemeets = $DB->get_records_sql($sql);
				if(count($googlemeets)> 0)
				{
					$sql ="delete from {event} where modulename='googlemeet' and  instance='".$googlemeet->id."' and 
					   timemodified <='".$googlemeet->timemodified."' and  timesort > now()";
					   $DB->execute($sql);
				}
				
				$sql = "select * from {googlemeet_events} where googlemeetid=".$googlemeet->id;
				$googlemeets_events = $DB->get_records_sql($sql);
				
				foreach($googlemeets_events as $googlemeets_event)
				{
				//	print_object(date("Y-m-d H:i:s",$googlemeets_event->eventdate));
					if($googlemeets_event->eventdate > time() )
					{
						
						$eventarr = Array();
						$eventarr['name'] = $googlemeet->originalname;
						$eventarr['description'] = $googlemeet->originalname;
						$eventarr['format'] = 1;
						$eventarr['categoryid'] = 0;
						$eventarr['courseid'] = $mycourse->id;
						$eventarr['groupid'] = 0;
						$eventarr['userid'] = 0;
						$eventarr['repeatid'] = 0;
						$eventarr['component'] = 0;
						$eventarr['modulename'] = 'googlemeet';
						$eventarr['instance'] = $googlemeet->id;
						$eventarr['type'] = CALENDAR_EVENT_TYPE_ACTION;
						$eventarr['eventtype'] = 'meetingtime';
						$eventarr['timestart'] = strtotime(date("Y-m-d",$googlemeets_event->eventdate)." ".$googlemeet->starthour.":".$googlemeet->startminute);
						$eventarr['timeduration'] = $googlemeets_event->duration;
						$eventarr['timesort'] = strtotime(date("Y-m-d",$googlemeets_event->eventdate)." ".$googlemeet->endhour.":".$googlemeet->endminute);
						$eventarr['visible'] = 1;
						$eventarr['uuid'] = '';
						$eventarr['sequence'] = 1;
						$eventarr['timemodified'] = $googlemeet->timemodified;
						//echo date("Y-m-d H:i:s",$eventarr['timestart']);
						
						
						$event = (object)$eventarr;
						
						 $params = ['modulename' => 'googlemeet', 'instance' => $googlemeet->id, 'eventtype' => 'meetingtime','courseid'=>$mycourse->id,'timestart'=>$eventarr['timestart']];
						 $rsevent = $DB->get_field('event', 'id', $params);
		...
		/************ added by Pankaj Wagh dt 30/05/2021 ********/
