...
class raw_event_retrieval_strategy implements raw_event_retrieval_strategy_interface {
    ...
    protected function get_raw_events_legacy_implementation(
		...
		foreach($mycourses  as $mycourse)
		{
			...
			{
				global $USER;
				//$DB->delete_records('event', array('modulename' => 'googlemeet', 'instance' => $googlemeet->id));
				
				$start = date_create(date("Y-m-d",$googlemeet->eventenddate)." ".$googlemeet->starthour.":".$googlemeet->startminute);
					$end = date_create(date("Y-m-d",$googlemeet->eventdate)." ".$googlemeet->endhour.":".$googlemeet->endminute);
				...
				$eventarr['timestart'] = strtotime(date("Y-m-d",$googlemeet->eventenddate)." ".$googlemeet->starthour.":".$googlemeet->startminute);
				$eventarr['timeduration'] = $diff->h*3600+$diff->m*60;
				$eventarr['timesort'] = strtotime(date("Y-m-d",$googlemeet->eventenddate)." ".$googlemeet->endhour.":".$googlemeet->endminute);
				$eventarr['visible'] = 1;
				$eventarr['uuid'] = '';
				$eventarr['sequence'] = 1;
				...
				$event = (object)$eventarr;
				 ...
				 $params = ['modulename' => 'googlemeet', 'instance' => $googlemeet->id, 'eventtype' => 'meetingtime','courseid'=>$mycourse->id];
