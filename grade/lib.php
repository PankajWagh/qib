...
function grade_print_tabs($active_type, $active_plugin, $plugin_info, $return=false) {
    ...
    foreach ($plugin_info as $plugin_type => $plugins) {
		...
		if($plugin_type=='export')
		{
			$ods = array_shift($plugins);
			$plugins['ods']=$ods;
			
			$url = $CFG->wwwroot.'/grade/export/xls/index.php?id='.$COURSE->id;
		}
		else
		{
			$first_plugin = reset($plugins);
    ...
    public static function get_user_profile_fields($courseid, $includecustomfields = false) {
        ...
        if (!empty($userprofilefields)) {
            foreach ($userprofilefields as $field) {
                ...
                if (in_array($field, $hiddenfields) || !in_array($field, $userdefaultfields) || $field =='lastname') {
                    continue;
                }
                $obj = new stdClass();
                $obj->customid  = 0;
		...
		$customprofilefields[]='department';
		$customprofilefields[]='division';
		$customprofilefields[]='group';
