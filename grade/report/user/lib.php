        ...
        if ($type == 'item' or $type == 'categoryitem' or $type == 'courseitem') {
            ...
            if (!$hide) {
                ...
                **/
				...
				/******* added by Pankaj Wagh *** 29/05/2021 ******/
				$completion_attempt=Array();
				if($grade_object->itemmodule=='scorm')
				{
					$scorm = $DB->get_record('scorm', array('id' =>$gradeitemdata['iteminstance']  ), '*', MUST_EXIST);
					
					require_once($CFG->dirroot.'/mod/scorm/locallib.php');
					$attemptids = scorm_get_all_attempts($scorm->id, $this->user->id);
					...
					foreach ($scoes as $sco) {
						if ($sco->launch != '') {
							...
							{
								$trackdata = scorm_get_tracks($sco->id, $this->user->id, $attempt);
								$completion_attempt[]=" Attempt ".$attempt ." ".$trackdata->status;
							}
                ...
                $data['status']['headers'] = "$header_cat $header_row average";
                $data['status']['content'] = implode("<br>",$completion_attempt);
				/******* added by Pankaj Wagh *** 29/05/2021 ******/
