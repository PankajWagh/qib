...
abstract class grade_report {
    ...
    public function setup_users() {
		...
		//print_object($SESSION->gradereport);
	
		
		if (isset($SESSION->gradereport['search']) && !empty($SESSION->gradereport['search'])) 
		{
			$this->userwheresql .= ' AND '.$DB->sql_like('u.firstname', ':searchval', false, false);
            $this->userwheresql_params['searchval'] = '%'.$SESSION->gradereport['search'].'%';
		}
		if (isset($SESSION->gradereport['department']) && !empty($SESSION->gradereport['department'])) 
		{
			$this->userwheresql .= ' AND u.id  in (select userid FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
									where shortname ="department" and  data =:departmentval) ';
											
            $this->userwheresql_params['departmentval'] =$SESSION->gradereport['department'];
		}
		
		if (isset($SESSION->gradereport['division']) && !empty($SESSION->gradereport['division'])) 
		{
			$this->userwheresql .= ' AND u.id  in (select userid FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
									where shortname ="division" and  data =:divisionval) ';
											
            $this->userwheresql_params['divisionval'] =$SESSION->gradereport['division'];
		}
		if (isset($SESSION->gradereport['group']) && !empty($SESSION->gradereport['group'])) 
		{
			$this->userwheresql .= ' AND u.id  in (select userid FROM {user_info_field} f
                                            JOIN {user_info_data} d ON d.fieldid = f.id
									where shortname ="group" and  data =:groupval) ';
											
            $this->userwheresql_params['groupval'] = $SESSION->gradereport['group'];
		}
		if (isset($SESSION->gradereport['status']) && !empty($SESSION->gradereport['status'])) 
		{
			/**/
			
			$sql='(SELECT distinct sst.userid from {scorm_scoes_track} sst inner join {scorm} scm on sst.scormid = scm.id where element="cmi.core.lesson_status" and lower(value) =lower(:status)  and  scm.course=:courseid) UNION  (select userid from {course_completions} where timecompleted > 0 and  course =:coursecompletionid)';
			$params_sql = array();
			$params_sql['status'] = 'passed';
			$params_sql['courseid'] = $SESSION->gradereport['courseid'];
			$params_sql['coursecompletionid'] = $SESSION->gradereport['courseid'];
			
			$recs = $DB->get_records_sql($sql,$params_sql);
			
			
			
			
			if(strtolower($SESSION->gradereport['status']) =='completed' && count($recs) > 0)
			{
				$this->userwheresql .= ' and  u.id  in ('.implode(",",array_keys($recs)).') ';								
				/*$this->userwheresql_params['status'] = 'passed';
				$this->userwheresql_params['courseid'] = $SESSION->gradereport['courseid'];
				$this->userwheresql_params['coursecompletionid'] = $SESSION->gradereport['courseid'];*/
			
			}
			else if(strtolower($SESSION->gradereport['status']) =='notcompleted' && count($recs) > 0)
			{
						
				$this->userwheresql .= ' and  u.id not in ('.implode(",",array_keys($recs)).') ';
			}
			if(strtolower($SESSION->gradereport['status']) !='notcompleted' && strtolower($SESSION->gradereport['status']) !='All' )
			{
				$timestart =  $timeend = '';
				if(isset($SESSION->gradereport['completiontimestart']) && !empty($SESSION->gradereport['completiontimestart']) !='' && isset($SESSION->gradereport['completiontimeend']) && !empty($SESSION->gradereport['completiontimeend']) !='')
				{
					$timestart =  make_timestamp($SESSION->gradereport['completiontimestart']['year'], $SESSION->gradereport['completiontimestart']['month'], $SESSION->gradereport['completiontimestart']['day'], 0,0);
					
					$timeend =  make_timestamp($SESSION->gradereport['completiontimeend']['year'], $SESSION->gradereport['completiontimeend']['month'], $SESSION->gradereport['completiontimeend']['day'], 0,0);
					
				}
				else if(isset($SESSION->gradereport['completiontimestart']) && !empty($SESSION->gradereport['completiontimestart']) !='' )
				{
					$timestart =  make_timestamp($SESSION->gradereport['completiontimestart']['year'], $SESSION->gradereport['completiontimestart']['month'], $SESSION->gradereport['completiontimestart']['day'], 0,0);
					
				}
				else if(isset($SESSION->gradereport['completiontimeend']) && !empty($SESSION->gradereport['completiontimeend']) !='' )
				{
					$timeend =  make_timestamp($SESSION->gradereport['completiontimeend']['year'], $SESSION->gradereport['completiontimeend']['month'], $SESSION->gradereport['completiontimeend']['day'], 0,0);
				...
				{
					$this->userwheresql .= ' and  u.id  in (select userid from {course_completions} where timecompleted > 0 and  course =:courseid and timecompleted >= :timestart and timecompleted <= :timeend ) ';
				
					$this->userwheresql_params['courseid'] = $SESSION->gradereport['courseid'];
					$this->userwheresql_params['timestart'] = $timestart;
					$this->userwheresql_params['timeend'] = $timeend;
					
				}
				else if($timestart != '' && $timeend == '')	
				{
					$this->userwheresql .= ' and  u.id  in (select userid from {course_completions} where timecompleted > 0 and  course =:courseid and timecompleted >= :timestart  ) ';
				...
				if($timestart == '' && $timeend != '')	
				{
					$this->userwheresql .= ' and  u.id  in (select userid from {course_completions} where timecompleted > 0 and  course =:courseid and timecompleted <= :timeend ) ';
		...
		//echo "****************".$this->userwheresql;
		//print_object( $this->userwheresql_params);
