...
class grade_report_grader extends grade_report {
    ...
    public function load_users() {
	   ...
	   list($enrolledsql, $enrolledparams) = get_enrolled_sql($this->context, '', 0, false);
        ...
        $sql = "SELECT $userfields,username
                  FROM {user} u
    ...
    public function get_left_rows($displayaverages) {
		...
		$extrafields[]='profile_field_department';
		$extrafields[]='profile_field_division';
		$extrafields[]='profile_field_group';
		//$extrafields[]='timeenrolled';
		$extrafields[]='timestarted';
		$extrafields[]='timecompleted';
		$extrafields[]='attempt';
		$extrafields[]='status';
		...
		foreach ($this->users as $userid => $user) {
			...
			/*********** added by Pankaj Wagh ******** dt. 02/09/2021 ************/
					...
					$sql ='SELECT f.id, f.shortname, f.datatype, d.data
                                           ...
                                           WHERE d.userid = ?';
					$user_profiles = $DB->get_records_sql($sql, array( $user->id));
					...
					//print_object($user_course_completion);
					
					/*	if($user_course_completion->timeenrolled > 0)
						{
							$user->timeenrolled=   userdate($user_course_completion->timeenrolled, get_string("strftimedatetime", "langconfig"));
						} */
					...
					if(trim($user->timestarted) !='' && $user->status =='')
					{
						$user->status ='Incomplete';
					}
					
			/*********** added by Pankaj Wagh ******** dt. 02/09/2021 ************/
			$arr_users[$user->id] = (Array)$user;
			$arr_sorts[$user->id]= $user->{$this->sortitemid};
		}
		
		$arr_users = $this->array_msort($arr_users, array($this->sortitemid=>( $this->sortorder == 'ASC' ? SORT_ASC :SORT_DESC) ));
		
			
		
		
		$this->users = $arr_users;
    ...
    public function get_sort_arrows(array $extrafields = array()) {
        ...
        foreach ($extrafields as $field) {
            ...
            if ($field == $this->sortitemid) {
              $arrows[$field] .=( $this->sortorder == 'ASC' ? $iconasc : $icondesc);
            }
        }
		//echo $this->sortitemid;
		//print_object($arrows);;
        return $arrows;
    }
