...
$sortitemid    = optional_param('sortitemid', 0, PARAM_RAW); // sort by which grade item
...
#graderreportsdepartment

if (isset($graderreportsdepartment) && !empty($graderreportsdepartment)) {
    $SESSION->gradereport['department'] = $graderreportsdepartment;
}
else
{
	unset($SESSION->gradereport['department']);
}
if (isset($graderreportsdivision) && !empty($graderreportsdivision)  ) {
    $SESSION->gradereport['division'] = $graderreportsdivision;
}else
{
	unset($SESSION->gradereport['division']);
}
if (isset($graderreportsgroup) && !empty($graderreportsgroup)) {
    $SESSION->gradereport['group'] = $graderreportsgroup;
}else
{
	unset($SESSION->gradereport['group']);
}
if (isset($graderreportsstatus) && !empty($graderreportsstatus)) {
    $SESSION->gradereport['status'] = $graderreportsstatus;
}else
{
	unset($SESSION->gradereport['status']);
}
if (isset($graderreportssearch) && !empty($graderreportssearch)) {
    $SESSION->gradereport['search'] = $graderreportssearch;
}
else
{
	unset($SESSION->gradereport['search']);
}
if (isset($courseid) && !empty($courseid)) { 
    $SESSION->gradereport['courseid'] = $courseid;
}
else
{
	unset($SESSION->gradereport['courseid']);
}

if (isset($completiontimestart)) {
    $SESSION->gradereport['completiontimestart'] = $completiontimestart;
}

if (isset($completiontimeend)) {
    $SESSION->gradereport['completiontimeend'] = $completiontimeend;
}
...
$department  = isset($SESSION->gradereport['department']) ? $SESSION->gradereport['department'] : '';
$division  = isset($SESSION->gradereport['division']) ? $SESSION->gradereport['division'] : '';
$group  = isset($SESSION->gradereport['group']) ? $SESSION->gradereport['group'] : '';
$status  = isset($SESSION->gradereport['status']) ? $SESSION->gradereport['status'] : '';
$search  = isset($SESSION->gradereport['search']) ? $SESSION->gradereport['search'] : '';
...
$graderreportsgroup   = optional_param('group', null, PARAM_NOTAGS);
$graderreportsstatus   = optional_param('status', null, PARAM_NOTAGS);
$graderreportssearch   = optional_param('search', null, PARAM_NOTAGS);
...
require_once($CFG->dirroot.'/grade/report/grader/filtergradehtml.php');
$gradeform = new filtergradehtml_form(new moodle_url($PAGE->url, array()));
